...
-- Reserving the time slot and creating the appointment record should be inside the transaction
-- because both actions must succeed together to keep the database consistent.
-- Sending the confirmation notification should be outside the transaction because it is an
-- external action and should not prevent the booking from being saved if the notification fails.
...
-- Calling COMMIT inside the procedure commits the entire transaction,
-- including the caller’s other changes. This prevents the developer from rolling
-- back all operations together if something later fails.
...
-- return a value. post_payment() cannot be used inside a SELECT statement because
